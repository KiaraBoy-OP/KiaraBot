...
* Blacklisted @KiaraBot_Chat from ``Gcast``.
...
* Updated ``.alive`` & ``.kiara`` looks. See it yourself. There's something more regarding Alive message, just keep on reading....
...
* Now you can set multiple Alive pic for your ``.alive`` & ``.kiara`` command. To do this, do ``.svar ALIVE_PIC <telegraph links separated by a space>``. This will save your alive pic to DB and you'll get random pics in alive message.
